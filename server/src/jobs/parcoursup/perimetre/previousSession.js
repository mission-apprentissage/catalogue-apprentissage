...
const run = async () => {
  ...
  await cursor(
    Formation.find({
      ...
      parcoursup_previous_session: { $ne: true },
    }).select({
      cle_ministere_educatif: 1,
    }),
    async ({ cle_ministere_educatif }) =>
      await Formation.updateOne({ cle_ministere_educatif }, { parcoursup_previous_session: true })
  );
  ...
  await cursor(
    Formation.find({
      ...
      parcoursup_previous_session: { $ne: false },
    }).select({
      cle_ministere_educatif: 1,
    }),
    async ({ cle_ministere_educatif }) =>
      await Formation.updateOne({ cle_ministere_educatif }, { parcoursup_previous_session: false })
  );
};
