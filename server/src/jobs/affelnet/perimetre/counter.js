...
const run = async () => {
    ...
    ...(await Promise.all(
      Object.values(AFFELNET_STATUS).map(async (status) => ({
        [`statut "${status}"`]: {
          ...
          "catalogue non réglementaire": await Formation.countDocuments({
            affelnet_statut: status,
  ...
  const results = {
    total: {
      ...
      "catalogue non réglementaire": totalNonReglementaire,
