...
const FILE_PATH = "/data/uploads/export-candidature-relations.csv";

const afImportCandidatureRelations = async () => {
  logger.info({ type: "job" }, " -- AFFELNET | IMPORT CANDIDATURE RELATIONS : ⏳ -- ");
  ...
  try {
    ...
    await asyncForEach(data, async (item, index) => {
      try {
        ...
        const localites_formation = item["Localité des établissements d'accueil"];
        const uais_formation = item["UAI des établissements d'accueil"];
        const codes_formation = item["Offres associées"];
        ...
        logger.debug({ type: "job" }, `${siret_responsable} / ${siret_formateur}`);

        await CandidatureRelation.create({
    ...
    logger.info({ type: "job" }, `${count} relations importées !`);
    logger.info({ type: "job" }, " -- AFFELNET | IMPORT CANDIDATURE RELATIONS : ✅ -- ");
  } catch (error) {
    logger.error({ type: "job" }, " -- AFFELNET | IMPORT CANDIDATURE RELATIONS : ❌ -- ");
  }
};

module.exports = { afImportCandidatureRelations };
