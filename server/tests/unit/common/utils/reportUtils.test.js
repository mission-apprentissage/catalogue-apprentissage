...
describe(__filename, () => {
  ...
  it("should create 1 report only under 500 data", async () => {
    const date = Date.now();
  ...
  it("should create 2 reports because of chunks after 500 data", async () => {
    const date = Date.now();
    const type = "rcoImport";

    await storeByChunks(type, date, { title: "hello" }, "added", [...Array(1000).keys()]);

    const count = await Report.countDocuments({});
    assert.strictEqual(count, 2);
  });

  it("should create 3 reports because of chunks after 1000 data", async () => {
    const date = Date.now();
    const type = "rcoImport";

    await storeByChunks(type, date, { title: "hello" }, "added", [...Array(1001).keys()]);
