...
const RelationEmailResponsableModal = ({ isOpen, onClose, formation, relation, callback }) => {
  const initialRef = React.useRef();
  const toast = useToast();

  const updates_history = [
    ...relation.updates_history?.sort(sortDescending),
    ...(relation?.email_responsable
      ? [
          {
            from: {},
            to: {
  ...
  const {
    ...
    validationSchema: Yup.object().shape({
      email_responsable: Yup.string().email(),
    }),
    enableReinitialize: true,
    onSubmit: async ({ email_responsable }, { setSubmitting }) => {
      try {
        setSubmitting(true);
        await _put(`${CATALOGUE_API}/entity/relations/${relation._id}`, {
          email_responsable,
        });
        ...
        <ModalBody px={[4, 16]} pb={[4, 16]}>
          <form onSubmit={handleSubmit}>
            <FormControl
              ...
              isInvalid={touched.email_responsable && errors.email_responsable}
            >
              <FormLabel for="email_responsable" htmlFor="affelnet" mb={3} fontSize="epsilon" fontWeight={400}>
              ...
              />
              <FormErrorMessage>{errors.email_responsable}</FormErrorMessage>
            </FormControl>
            ...
            <UnorderedList>
              {updates_history?.map((history, index) => (
                <ListItem key={index} mb="8px">
                  {history.from?.email_responsable?.length > 0 && history.to?.email_responsable?.length > 0 && (
                    <>
                      Modification de l'adresse courriel
                      <Text as="span" variant="highlight" mx="0.5rem">
                        {history.from?.email_responsable ?? ""}
                      </Text>
                      à
                      <Text as="span" variant="highlight" mx="0.5rem">
                        {history.to?.email_responsable ?? ""}
                      </Text>
                    </>
                  )}
                  ...
                  {(!history.from?.email_responsable || history.from?.email_responsable?.length === 0) &&
                    history.to?.email_responsable?.length > 0 && (
                      <>
...
export { RelationEmailResponsableModal };
